...
int main() 
{
    // for(int i = 9; i < 5000; i += 10)
    // {
    string input;
    cin >> input;
    //cout << i << endl;
    bigint S = bigint(input);  

    bigint factor = bigint(400) + bigint(4 * 50) * S;
    bigint n_pre = (sqrt(factor) - bigint(20)) / bigint(100) * bigint(2);


    bigint S_pred = (n_pre / 2) * (n_pre / 2) * bigint(50) + bigint(20) * (n_pre / 2) + bigint(30) * (n_pre + 1);
    bigint n_res = 0;
    
    if (S_pred > S) {

        bigint S_pred_pred = S_pred;
        S_pred -= bigint(30) * (n_pre + 1);
        if (S_pred == S) {
            std::cout << n_pre << " " << S << std::endl;
        } else {
            S_pred = S_pred + (n_pre + 1) * bigint(10);
            if (S_pred < S) {
                S_pred = S_pred + (n_pre + 1) * bigint(10);
            }
            if (S_pred < S) {
            ...
            if (S_pred == S_pred_pred) {
                std::cout << n_pre + 1 << " ";
            } else {
                std::cout << n_pre << " ";
            }
            std::cout << S_pred << std::endl;
        }
    } else {
        n_pre = n_pre + 1;
        bigint S_pred_pred = S_pred + bigint(20) * (n_pre + 1);
        if (S_pred == S) {
            std::cout << n_pre << " " << S << std::endl;
        } else {
            S_pred = S_pred + (n_pre + 1) * bigint(10);
